...
import { DeviceWidth, DeviceHeight } from '../../Components/DeviceDeminsions/DeviceDeminsions';
...
const UserProfile = ({ navigation }) => {
  ...
  return (
    <React.Suspense fallback={<RowText fontColor="black">Loading</RowText>}>
      {productList ? <Modal style={styles.modal} isVisible={productList}>
        <ScrollView>
          {productElemets.map(product => <ListItem
            containerStyle={[{ backgroundColor: '#fff', elevation: 15, marginBottom: 1 }]}
            leftElement={
              <Images
                source={[{
                  uri: product.imageList[0],
            ...
            subtitle={
              <View style={{ flexDirection: 'row' }}>
                ...
                <RowText
                  ...
                  fontColor={'green'}
                  fontize={14}>
                  {`${RupeeSymbol} ${product.price ? product.price.sp : 'No Price Found'}`}
                </RowText>
              </View>
            }
            title={
            ...
            rightElement={<RowText fontColor="#000" fontize={12}>{`${RupeeSymbol} ${product.price.sp}`}</RowText>}

          />

          )}
        </ScrollView>
        <Button containerStyle={styles.btnPosition} titleStyle={{ color: ThemeYellow }} buttonStyle={{ backgroundColor: Darkest }} onPress={() => setProductList(false)} title={`CLOSE ${RupeeSymbol} ${totalCart}`} />

      </Modal> :
        isModalVisible ? <UserProfileUpdate isShow={true} /> : <View style={{ flexDirection: 'column', padding: 15 }}>
          <View style={{ flexDirection: 'row', marginBottom: 9 }}>
            <Avatar
              ...
              title={userProfile && userProfile["name"] && userProfile["name"].toString().substring(0, 1)}
            // showAccessory
            />
            <View style={{ flexDirection: 'column', marginLeft: 20, marginTop: 10 }}>
              ...
              <RowText fontColor={'grey'} fontFormat={'Italic'} fontize={12}>
                {userProfile && userProfile['addresses'] && userProfile.addresses.length > 0 && userProfile['addresses'][0]['area'] || ''}
              </RowText>
            </View>
          </View>
          ...
          <View
            ...
            <RowText fontColor={'black'} paddingLeft={10} fontize={12} style={{ marginBottom: -20 }}>
              {userProfile && userProfile.name}
            </RowText>
            <RowText
              paddingLeft={15}
            ...
            ${userProfile && userProfile.addresses[0]["houseNumber"]}, ${userProfile.addresses[0]["area"]} 
          ...
          <ScrollView horizontal={true} showsHorizontalScrollIndicator={false}>
            {cartElemets && cartElemets.map(e => <TouchableOpacity onPress={() => showProductsList(e.products, e.total)}><View
              style={{
                width: 220,
                height: 110,
                borderWidth: 0,
              ...
              <RowText fontColor={'black'} paddingLeft={10} fontize={14} style={{ marginTop: 10 }}>
                Total Items : {`${e && e.products && e.products.length}`}
              </RowText>
              <RowText fontColor={'black'} fontFormat="Italic" paddingLeft={10} fontize={14} style={{ marginTop: 0 }}>
                Order Date: {` ${moment(e.orderDate).format('DD-MM-YY  hh:mm')}`}
              </RowText>
              <RowText
                paddingLeft={15}
                fontFormat="Italic"
